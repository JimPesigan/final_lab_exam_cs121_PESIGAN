		...
		if not os.path.exists("data.txt"):
			...
			with open('data.txt', 'r') as f:
				for line in f:
					...
					wins = values[2]
					ID = values[3]
					scores.append(Score(username, points, wins, ID))

	def save_scores(self):
		with open('data.txt', 'w') as f:
			for i in range(len(scores)):
				f.write(f"{scores[i].username}, {scores[i].points}, {scores[i].wins}, {scores[i].ID}\n")
		...
		while True:
			if self.rolls(username) == True:
				...
				if choice == 1:
					...
					scores.append(Score(username, self.points, self.wins, ID))
					self.save_scores()
					self.points = 0
					self.win = 0
					return
	...
	def rolls(self, username):
			...
			while RoundWin < 2 and RoundLose < 2:
				...
				if RoundWin == 2:
						...
						self.points += 3
						self.wins += 1
						print(f"""{username} Total points: {self.points}, Stages Won: {self.wins}""")
